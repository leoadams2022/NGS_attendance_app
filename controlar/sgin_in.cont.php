    ...
    if($Validate =='allGood'){
            ...
            if(!empty($user)){
                    ...
                    if(password_verify($_POST['pass'], $hashed_password)){
                        ...
                        //for cookies tokens 
                        $rem_me = 'dont_add_token';
                        if($_POST['remember_me'] === 'true'){
                            // $rem_me = 'add_token';
                            $token_class = new Token_Class();
                            $adding_token = $token_class->add_remember_me_token($user['user_name']);
                        }
                        //-----------
                        if($user['rank'] ==='admin'){
                            $results = ['state'=> 'good','msg'=> 'sgined in successfully','url'=> 'admin/Dashboard.admin.php','respond'=>$rem_me ];
                            echo json_encode($results);
                        }else{
                        $results = ['state'=> 'good','msg'=> 'sgined in successfully','url'=> 'Dashboard.php','respond'=> $rem_me ];
                        echo json_encode($results);
                        }
                    }else{
