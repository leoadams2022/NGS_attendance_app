...
if(isset( $_POST['timeZone'])&&isset($_POST['needTo'])){
    ...
    if($needTo === 'in'){
        ...
        $results = ['state'=> 'good','msg'=> $adding_time_in['msg'],'url'=> '','respond'=>$adding_time_in['respond']];
        ...
        if($_POST["if_late"] === 'true'){
          $Announcements_Class = new Announcements_Class();
          $user_class = new Users_Class();
          $adminsUserNames = $user_class->get_all_adminsUserNames();
          if($adminsUserNames != 'no data found'){
            $add_late_alert = $Announcements_Class->alert_admin_agent_late($user_name,$adminsUserNames,'in');
          }
        }
    }elseif($needTo === 'out'){
        $timeZone = $_POST['timeZone'];
        ...
        if($_POST["if_late"] === 'true'){
          ...
          $adminsUserNames = $user_class->get_all_adminsUserNames();
          if($adminsUserNames != 'no data found'){
            $add_late_alert = $Announcements_Class->alert_admin_agent_late($user_name,$adminsUserNames,'out');
          }
        }
      }
...
if(isset($_POST['get_attendance_log'])&&isset($_POST['needTo'])&&isset($_POST['Thismonth'])){
  if($_POST['needTo'] === 'get_attendance_log'){
    ...
    if($get_attendance_log_for_user === 'no data'){
      $results = ['state'=> 'bad','msg'=> 'there is no attendance log found','url'=> '','respond'=>$in_out_timeing];
      echo json_encode($results);
    }else{
...
if(isset($_POST['needTo'])){
  if($_POST['needTo'] === 'add_excuse'){
    $Attendance_Class = new Attendance_Class();
    $excuse = $Attendance_Class->add_excuse_by_id($_POST['entry_id'],$_POST['excuse_text']);
    $results = ['state'=> 'good','msg'=> '' ,'url'=> '','respond'=>$excuse];
    echo json_encode($results);
  }
}
