    ...
    <table class="table table-striped table-hover attendancce_log_table" id="attendancce_log_table">
                  ...
                  <tr class="table-primary text-center">
                    ...
                    <td class="text-center">
                        <!-- 12:39:53 AM -->
                        <input class="form-control" type="time" id="in_time" name="appt_edit" value=''>
                    </td>

                    <td class="text-center">
                        <!-- 2:00:00 AM -->
                        <input class="form-control" type="time" id="out_time" name="appt_edit" value=''>
                    </td>

                    <td class="text-center">
                        <input class="form-control" type="text" id="excuse" name="appt_edit" value=''>
                    </td>
                </tr>
                  </tbody>
        </table>
...
<script>
    ...
    if (typeof(excuse) === undefined) {
        let excuse = '<?=$_GET['excuse']?>';
    }else{
        excuse = '<?=$_GET['excuse']?>';
    }
    $('#excuse').val(excuse.replace(/_/g, " "));
...
// console.log(out_only_date,in_only_date);
 ...
 $('#submit_btn').click(function (){
    ...
    // console.log(excuse1);
    let out_notSet_chack = document.getElementById('out_time').value;
    let time_out_string;
    if(out_notSet_chack === ''){
        time_out_string = 'NULL';
    }else{
        time_out_string = out_only_date+' '+document.getElementById('out_time').value;//$('#out_time').val()
    }
    $.post('../controlar/attendance.cont.admin.php',
            ...
            new_outTime: time_out_string,//$('#out_time').val()
            new_excuse: excuse1
            },
