...
if(isset($_POST['needTo'])){
    if($_POST['needTo'] === 'get_all_agints'){
        ...
        $new_excuse;
        $new_outTime;
        if($_POST['new_excuse'] === 'NULL'){ $new_excuse =NULL; }else{ $new_excuse =$_POST['new_excuse'];}
        if($_POST['new_outTime'] === 'NULL'){ $new_outTime =NULL; }else{ $new_outTime =$_POST['new_outTime'];}
        $update_entry = $attendance_class->update_entry($_POST['entry_id'],$_POST['new_inTime'],$new_outTime,$new_excuse);
