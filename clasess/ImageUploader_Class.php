...
class ImageUploader extends Crud {
  private $upload_dir;
  public $root = 'http://localhost/LoginAgain/';
  ...
  public function uploadImage($file,$name) {
    ...
    if ($file['error'] !== UPLOAD_ERR_OK) {
      ...
      return array('state'=>'bad','msg'=>'An error occurred while uploading the file.');
    }
    ...
    if (strpos($mime_type, 'image/jpeg') !== 0) {
      ...
      return array('state'=>'bad','msg'=>'The uploaded file is not an image.');
    }
    ...
    if ($this->imageExists($new_filename)) {
        ...
        $update_img = $this->updateImage($file, $new_filename,$name);//$username
        return $update_img;
        // end();
    }
    ...
    return array('state'=>'good','url'=>$this->root . 'images/' . $new_filename,'msg'=>'photo added');
  }
  ...
  public function updateImage($file, $filename,$username) {
    ...
    if ($mime_type !== 'image/jpeg') {
      ...
      return array('state'=>'bad','msg'=>'Only JPEG images are allowed.');
    }
    ...
    return array('state'=>'good','url'=>$this->root . 'images/' . $new_filename,'msg'=>'photo updated');
  }
  ...
  public function delete_img($img_name){
    if($this->imageExists($img_name)){
      ...
      unlink($filepath);
      return array('state'=>'good','msg'=>'photo deleted');
    }
    return array('state'=>'bad','msg'=>'photo dose not exsist');
  }
