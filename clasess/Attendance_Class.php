...
class Attendance_Class extends Crud {
   ...
   public function im_In($timeZone='Etc/GMT-2',$user_name,$user_id){
    ...
    public function get_attendance_month_user($month,$user_name,$user_id){
        ...
        $data = $this->query("SELECT  `id`, `day_date`, `is_it_over`, `time_in`, `time_out`, `excuse` FROM `attendance` WHERE MONTH(day_date) = $month AND `user_name` = '$user_name' AND `user_id` = $user_id ORDER BY `day_date`");
        if(empty($data)){
            return 'no data';//
        }else{
            return $data;// 
    ...
    public function update_entry($entry_id,$new_inTime,$new_outTime,$new_excuse){
        //Updating DATA
        $is_it_over;
        if($new_outTime == NULL){ $is_it_over = 'no';}else{$is_it_over = 'yes';}
    ...
    public function add_excuse_by_id($id,$excuse_text){
        $update = $this->save(array(
            'table'  => 'attendance',
            'id'  => $id, // you need to pass a id (int)
            'excuse' => $excuse_text
        ...
        if($update !==FALSE){
            return "successfull"; // data is updated successfully
        }
